...
which_study = 8;    %studies 1, 2, 3 and 4 are in fit_models_matlabStudies.m. So start with %study 5: face 2 (online) 6: foods 1 (online) 7: holidays 1 (0nline) 8: face 3 (open day) 9: foods 3 (open day) 10: holidays 3 (open day) 
check_params = 1;       %fit the same model that created the data and output estimated parameters
make_est_model_data = 1;
use_file = 1; %Set the above to zero and this to 1 and it'll read in a file you specify (See filename_for_plots variable below) and make plots of whatever analyses are in the Generate_params structure in that file;
...
% filename_for_plots = [outpath filesep 'out_imageTask_gorilla_study05_20231410.mat'];
filename_for_plots = [outpath filesep 'out_imageTask_gorilla_study08_20231410.mat'];
...
if check_params == 1;
    ...
    [mean_ratings_all seq_vals_all output_all] = get_sub_data(which_study);
    ...
    fitting_bounds.BP = [-100 100];  %biased prior, value can't exist outside the rating scale
    fitting_bounds.Opt = [-100 100];  %optimism, value can't exist outside the rating scale
    fitting_bounds.BV = [-100 100];  %biased variances, can't be wider than the whole rating scale
    fitting_bounds.beta = [0 Inf];   %if not sometimes produces crazy values in the thousands, though rarely
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [mean_ratings seq_vals output] = get_sub_data(which_study);

if which_study == 5 | which_study == 6 | which_study == 7;
...
elseif which_study == 8 | which_study == 9 | which_study == 10;
...
if which_study == 5 | which_study == 8; %face domains
    
    %A little different from the others because there are separate male/female lists
    ...
    data = [temp_r(~isnan(temp_r(:,5)),5:6); temp_r(~isnan(temp_r(:,7)),7:8)];  %this time, concatenate columns for females, then males
...
elseif which_study == 6 | which_study == 9; %foods domains
...
elseif which_study == 7 | which_study == 10; %face domains
