...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = imageTask_paper_parameter_recovery_v3; 
...
% do_models = [2 1 4 7 5];    %These are now what v2 called model identifiers - Applies at the moment to both make_model_data and check_params;
do_models = [2];    %These are now what v2 called model identifiers - Applies at the moment to both make_model_data and check_params;

comment = 'hybrid_newerVB_noBV';    %The filename will already fill in basic parameters so only use special info for this.
...
% filename_for_plots = [outpath filesep 'out_PR_180models5modelTypes6paramLevels6betaLevels10subs5seqs12opts_hybrid_startover_20232411.mat'];
num_param_levels = 3;   %Will use start values and increments specified below to run up to this many parameters for each model
% fit_betas = [1:10:101];
fit_betas = [1];
...
if simulate_stimuli == 1;
    ...
    Generate_params.num_subs = 3;   %So this will be per parameter value
    Generate_params.num_seqs = 5;
    Generate_params.seq_length = 12; %hybrid might've usually used 12 but imageTask always uses 8!
    Generate_params.num_vals = 90;  %How many items in phase 1 and available as options in sequences? I've used before 426 or 90
...
%         identifiers = [identifiers i*ones(1,total_params_per_model)];
...
%     the_param_levels(4,:) = linspace(30,70,num_param_levels); param_used(4) = 7;                   %Model indicator 4: BV
%     the_param_levels(5,:) = linspace(30,80,num_param_levels); param_used(5) = 9;                   %Model indicator 5: BR
%     the_param_levels(6,:) = linspace(-20,20,num_param_levels); param_used(6) = 10;                  %Model indicator 6: Biased prior (obsolete)
%     the_param_levels(7,:) = linspace(-9,9,num_param_levels); param_used(7) = 11;                    %Model indicator 7: Opt
%     the_param_levels(8,:) = linspace(-30,50,num_param_levels); param_used(8) = 12;                  %Model indicator 8: Biased variance (obsolete)
% 
% %sequence length 12
if Generate_params.seq_length == 12;
   the_param_levels(1,:) = linspace(2,Generate_params.seq_length-1,num_param_levels); param_used(1) = 4;   %Model indicator 1: Cut off;
    the_param_levels(2,:) = linspace(-.05,.02,num_param_levels)*100; param_used(2) = 5;              %Model indicator 2: Cs
    the_param_levels(4,:) = linspace(40,80,num_param_levels); param_used(4) = 7;                   %Model indicator 4: BV
    the_param_levels(5,:) = linspace(35,90,num_param_levels); param_used(5) = 9;                   %Model indicator 5: BR
    the_param_levels(6,:) = linspace(-20,20,num_param_levels); param_used(6) = 10;                  %Model indicator 6: BP
    the_param_levels(7,:) = linspace(-15,15,num_param_levels); param_used(7) = 11;                    %Model indicator 7: Opt
...
%     end;   %loop through betas (beta_val) and assign their parameter configurations
...
if check_params == 1;
    ...
    fitting_bounds.Cs = [-10 10];   %cost to sample
