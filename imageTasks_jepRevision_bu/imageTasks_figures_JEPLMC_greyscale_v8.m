...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = imageTasks_figures_JEPLMC_greyscale_v8;

%26th June 2026:v8: optional BIC-weight model-support plot and corrected BIC penalty switch.
%26th June 2026:v7: single greyscale bars/points; heatmaps retain bluish colormap.
...
% Model-support panel switch.
% 'hard'       = original hard winner counts (one winning model per participant).
% 'bic_weight' = summed participant-level BIC weights, i.e. soft model support.
% Note: confusion-matrix correction is only applied to hard winner counts.
% model_support_metric = 'hard';  % 'hard' or 'bic_weight'
model_support_metric = 'bic_weight';  % 'hard' or 'bic_weight'


% BIC convention switch.
% Correct BIC is k*log(n) + 2*NLL, where n is the number of modelled binary choices.
% The older plotting code used k*n + 2*NLL. This did not change comparisons
% among the one-parameter models, because k was identical, but it is not the BIC formula.
use_correct_BIC = 1;
...
New_struct.correct_freqs = correct_freqs;
...
% Single-greyscale palette for all bars and points. Model identity is
% indicated by the x-axis labels, not by shade; this avoids creating a
% redundant greyscale code that would require a legend.
...
base_grey = 0.25;
plot_details.plot_cmap = repmat([base_grey base_grey base_grey], 9, 1);
    ...
    for this_bar = 1:plot_details.num_models_per_study;
        ...
        if Generate_params.study(study).model(this_bar).skip == 0; %If a theoretical model and not participants or IO
            ...
            elseif Generate_params.IC == 2; %If BIC (per participant)
                % Number of modelled binary choices for this participant.
                % num_samples is the participant's mean samples to decision; num_seqs is
                % the number of sequences contributing data. Their product therefore
                % recovers the total number of choice observations entering the likelihood.
                n_obs = Generate_params.study(study).num_samples.*Generate_params.study(study).num_seqs;
                n_obs(n_obs < 1) = 1; % safety, avoids log(0)
                if isfield(Generate_params,'use_correct_BIC') && Generate_params.use_correct_BIC == 1
                    IC_pps = no_params.*log(n_obs) + 2*lla;
                else
                    % Legacy behaviour, retained only for checking that old figures are reproducible.
...
%             end;
    ...
    %winning/support models
    [a pps_indices] = min(IC_pps_all_models');
    hard_win_counts = histcounts(pps_indices, 1:size(IC_pps_all_models,2)+1);
    BIC_weights = bic_weights_from_ic(IC_pps_all_models);

    if isfield(Generate_params,'model_support_metric') && strcmpi(Generate_params.model_support_metric,'bic_weight')
        % Soft model support: each participant contributes BIC weights that sum to 1.
        % This is useful when hard wins may overstate heterogeneity due to tiny BIC differences.
        win_counts = nansum(BIC_weights,1);
        support_ylabel = 'Summed BIC weight';
        fprintf('Study %d summed BIC weights: ', study);
        for m = 1:numel(IC_labels_all_models)
            fprintf('%s = %.2f; ', char(IC_labels_all_models(m)), win_counts(m));
        end
        fprintf('\n');
    else
        % Original hard winner counts. The confusion-matrix correction is only meaningful here.
        win_counts = hard_win_counts;
        if isfield(Generate_params,'correct_freqs') && Generate_params.correct_freqs == 1
            win_counts = adjust_freqs(win_counts);
        end
        support_ylabel = 'Best-fitting participants';
    end
...
%         bar(model,numel(find(pps_indices==model)), ...
        ...
        ylabel(support_ylabel);
        
    end;    %models
    

    %analyse model frequencies/support before plotting "significance" lines
    if isfield(Generate_params,'model_support_metric') && strcmpi(Generate_params.model_support_metric,'bic_weight')
        % These are bounded/compositional, so treat as descriptive/supporting tests.
        bic_weight_comparison(BIC_weights, IC_labels_all_models, study);
...
%             end;  %"significant"?
    ...
    % Keep the heatmap using MATLABs default/previous bluish colormap;
    % do not inherit the manuscript greyscale palette here.
...
% Participant-level BIC/AIC weights from an N participants x M models IC matrix.
% Rows sum to 1. For BIC, these are approximate posterior model probabilities
% under equal model priors, using log model evidence approx. -0.5*BIC.

    dIC = IC_matrix - min(IC_matrix, [], 2);
    W = exp(-0.5*dIC);
    W = W ./ sum(W, 2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function bic_weight_comparison(W, model_labels, study)
% Lightweight repeated-measures checks on participant-level BIC weights.
% These are intended as descriptive support analyses, not as a replacement for BIC/BMS.

    fprintf('Study %d mean BIC weights: ', study);
    meanW = nanmean(W,1);
    for m = 1:numel(model_labels)
        fprintf('%s = %.3f; ', char(model_labels(m)), meanW(m));
    end
    fprintf('\n');

    pairsW = nchoosek(1:size(W,2),2);
    for pw = 1:size(pairsW,1)
        m1 = pairsW(pw,1);
        m2 = pairsW(pw,2);
        try
            [bf10,pval] = bf.ttest(W(:,m1) - W(:,m2));
            fprintf('Study %d BIC-weight comparison %s vs %s: BF10 = %.2f, p = %.3f\n', ...
                study, char(model_labels(m1)), char(model_labels(m2)), bf10, pval);
        catch
            [~,pval] = ttest(W(:,m1), W(:,m2));
            fprintf('Study %d BIC-weight comparison %s vs %s: p = %.3f\n', ...
                study, char(model_labels(m1)), char(model_labels(m2)), pval);
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
