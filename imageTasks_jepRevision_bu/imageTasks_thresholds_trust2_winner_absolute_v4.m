function imageTasks_thresholds_trust2_winner_absolute_v4()
% Absolute-position threshold-shape analysis for Trustworthiness dataset 2.
% v4 adds raw versus within-participant-z plotting modes.
...
% Plot modes for the curve figures:
%   'raw'           plots continuation thresholds in raw action-value units.
%   'participant_z' z-scores thresholds within participant across positions 1-7 before plotting/fitting.
% Use {'raw'} or {'participant_z'} for one version, or both to compare.
cfg.plot_modes = {'raw','participant_z'};
...
writetable(Part, [cfg.outdir filesep 'trust2_threshold_winner_absolute_participant_means_v4.csv']);
writetable(Fits, [cfg.outdir filesep 'trust2_threshold_winner_absolute_fits_v4.csv']);
writetable(Summary, [cfg.outdir filesep 'trust2_threshold_winner_absolute_summary_v4.csv']);
save([cfg.outdir filesep 'trust2_threshold_winner_absolute_v4.mat'],'cfg','Tw','Part','Fits','Summary');

print_summary(Summary,cfg);
for pm = 1:numel(cfg.plot_modes)
    make_curve_fig(Part,Fits,cfg,'linear',cfg.plot_modes{pm});
    make_curve_fig(Part,Fits,cfg,'quadratic',cfg.plot_modes{pm});
end
...
function make_curve_fig(Part,Fits,cfg,kind,plot_mode)
% Plot absolute-position threshold trajectories.
%
% Rows:
%   1 = participant position means in grey + group-average fitted curve
%   2 = participant fitted curves in grey + group-average fitted curve
%
% Columns:
%   cost-to-sample winners, biased-prior winners
%
% plot_mode:
%   raw           = raw continuation-value threshold units
%   participant_z = within-participant z-scored thresholds before plotting/fitting
...
if strcmp(plot_mode,'raw')
    mode_label = 'raw';
    ylab = 'Continuation-value threshold';
elseif strcmp(plot_mode,'participant_z')
    mode_label = 'participant-z';
    ylab = 'Within-participant z-scored threshold';
else
    error('Unknown plot_mode: %s', plot_mode);
end

outfile = sprintf('trust2_threshold_winner_absolute_%s_curves_%s_v4', fitlab, strrep(mode_label,'-','_'));
x=cfg.pos; xd=linspace(min(x),max(x),200);

f=figure('Color','w','Name',[fitlab ' winner-model threshold curves: ' mode_label]);
t=tiledlayout(f,2,2,'TileSpacing','compact','Padding','compact');

for gi=1:numel(cfg.groups)
    group=string(cfg.groups{gi});
    subs=unique(Part.Subject(string(Part.BestFitModel)==group))';

    % Precompute participant plotting data and fitted curves in selected scale.
    P = make_plot_participant_curves(Part, subs, xd, kind, plot_mode);

    % ---------------------------------------------------------------------
    % Row 1: participant means in grey, group mean fitted curve on top.
    % ---------------------------------------------------------------------
    ax=nexttile(t,gi); hold(ax,'on'); box(ax,'off');
    for si=1:numel(P)
        plot(ax,P(si).x,P(si).y,'-','Color',[.78 .78 .78],'LineWidth',.75);
    end
    plot(ax,xd,P(1).group_y,'-','Color',cfg.cols(gi,:),'LineWidth',3);

    xlabel(ax,'Sequence position');
    ylabel(ax,ylab);
    title(ax,sprintf('%s\nparticipant means + mean %s fit',cfg.group_short{gi},fitlab));
    set(ax,'FontName','Arial','FontSize',9,'XTick',x);

    % ---------------------------------------------------------------------
    % Row 2: participant fitted curves in grey, group mean fitted curve on top.
    % ---------------------------------------------------------------------
    ax=nexttile(t,gi+2); hold(ax,'on'); box(ax,'off');
    for si=1:numel(P)
        plot(ax,xd,P(si).yfit,'-','Color',[.78 .78 .78],'LineWidth',.75);
    end
    plot(ax,xd,P(1).group_y,'-','Color',cfg.cols(gi,:),'LineWidth',3);

    xlabel(ax,'Sequence position');
    ylabel(ax,ylab);
    title(ax,sprintf('%s\nparticipant %s fits + mean fit',cfg.group_short{gi},fitlab));
    set(ax,'FontName','Arial','FontSize',9,'XTick',x);
end

saveas(f,[cfg.outdir filesep outfile '.png']);
print(f,[cfg.outdir filesep outfile '.tif'],'-dtiff','-r600');
end

function P = make_plot_participant_curves(Part, subs, xd, kind, plot_mode)

P = struct('x',{},'y',{},'yfit',{},'coef',{});
coefs = [];

for si=1:numel(subs)
    th=Part(Part.Subject==subs(si),:);
    [xx,oo]=sort(th.Position);
    yy=th.MeanThreshold(oo);

    if strcmp(plot_mode,'participant_z')
        yy = zscore_safe(yy);
    end

    coef = fit_curve_coefficients(xx, yy, kind);
    yfit = eval_curve(coef, xd);
    ...
    coefs(end+1,1:numel(coef)) = coef; %#ok<AGROW>
end

mean_coef = mean(coefs,1,'omitnan');
group_y = eval_curve(mean_coef, xd);

for si=1:numel(P)
    P(si).group_y = group_y;
end
end

function z = zscore_safe(y)
y = y(:)';
sd = std(y,'omitnan');
if isfinite(sd) && sd > 0
    z = (y - mean(y,'omitnan')) ./ sd;
else
    z = y*NaN;
end
end

function coef = fit_curve_coefficients(x,y,kind)
x=x(:); y=y(:);
valid=isfinite(x)&isfinite(y);
x=x(valid); y=y(valid);
if strcmp(kind,'linear')
    if numel(unique(x)) >= 2
        coef = polyfit(x,y,1);   % [slope intercept]
    else
        coef = [NaN NaN];
    end
else
    if numel(unique(x)) >= 3
        coef = polyfit(x,y,2);   % [quadratic slope intercept]
    else
        coef = [NaN NaN NaN];
    end
end
...
saveas(f,[cfg.outdir filesep 'trust2_threshold_winner_absolute_pointspread_v4.png']); print(f,[cfg.outdir filesep 'trust2_threshold_winner_absolute_pointspread_v4.tif'],'-dtiff','-r600');
end
