...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = imageTasks_figures_JEPLMC_v4_greyscale_weights_restored;
...
%     if num_models_per_study ~= 1;
        ...
        %get the pairs connecting participants to all non-human bars and set up their y axis locations
        %This restores the old behaviour: Bayesian pairwise lines in the
        %samples row compare Human with Ground truth and with each fitted model.
        all_indices = 1:size(samples_accum, 2);
        other_indices = all_indices(all_indices ~= human_is_index);
        num_pairs = numel(other_indices);
        y_inc = .45;

        %Place Bayesian comparison lines just above the sequence-length maximum.
        %Use a separate pair_idx because pair is an x-axis bar index, not a
        %1:num_pairs index.
        line_y_values = Generate_params.study(study).seq_length + y_inc*(1:num_pairs);
        ylim_top = Generate_params.study(study).seq_length + y_inc*(num_pairs + 1);
        
        %loop through pairs, get Bayes factors, plot them
        heatmap_mat = NaN(size(samples_accum,2));
        bf10 = NaN(1,num_pairs);
        samples_pvals = NaN(1,num_pairs);
        ...
        for pair = other_indices;
            ...
            %Compute paired Cohen's d for output
        ...
        end;   %pairs: Loop through comparisons between participants and all non-human bars
        %Set Y lim to include all Bayesian comparison lines, but keep y ticks
        %labelled only up to the real sequence length.
        set(gca,'Ylim',[0 ylim_top],'YTick',[0:2:Generate_params.study(study).seq_length]); box off;
