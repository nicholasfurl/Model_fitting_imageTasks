...
which_study = 4;    %1: av/face_1 %2: matchmaker %3: (small) trust 1  %4 (big) trust 2 (technically gorilla, not matlab)
...
filename_for_plots = 'C:\matlab_files\fiance\parameter_recovery\beta_fixed_code\Model_fitting_imageTasks\outputs\out_new_ll1_imageTask_matlab_study02_20231010.mat';
...
%     subjects = [101:116 118 119];    %small trust sub nums 
    subjects = [100:119];    %small trust sub nums 
...
%             num_samples(sequence,this_sub) = find(difVal<0,1,'first');  %assign output num samples for Bruno model
            ...
            num_samples(sequence,this_sub) = round(nanmean(samples_this_test));
            ranks(sequence,this_sub) = round(nanmean(ranks_this_test));
...
elseif which_study == 2; %matchmaker
    ...
    data_folder = 'C:\matlab_files\fiance\parameter_recovery\beta_fixed_code\Model_fitting_imageTasks\matchmaker_study\data';
    rating_files = dir( [data_folder filesep sprintf('av_ratings_sub%02d*.mat',subject)] );
    seq_files = dir( [data_folder filesep sprintf('av_sequence_sub%02d*.mat',subject)] );
    
elseif which_study == 3;    %small trust / trust 1
    
    data_folder = 'C:\matlab_files\fiance\parameter_recovery\beta_fixed_code\Model_fitting_imageTasks\trust_study_1_small\data';
...
[dummy1 dummy2 data{1}] = xlsread('C:\matlab_files\fiance\big_trust\all_big_trust_ratings_noaverage.xlsx');
...
[draws{1} dummy dummy] = xlsread('C:\matlab_files\fiance\big_trust\all_sequences_big_trust.xlsx');
...
[dummy dummy seq_fnames{1}] = xlsread('C:\matlab_files\fiance\big_trust\all_big_trust_sequence_fnames.xlsx');
