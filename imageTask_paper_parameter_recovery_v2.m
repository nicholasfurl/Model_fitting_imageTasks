...
filename_for_plots = [outpath filesep 'out_param_rec_ll1_40models20subs5seqs8opts_imageTask_bnds_20230910.mat'];
...
if make_config_model_data == 1;
    ...
    %Wider params:
    the_param_levels(1,:) = linspace(1,Generate_params.seq_length,num_param_levels); param_used(1) = 4;   %Model indicator 1: Cut off (integer values between first and last option);
...
if check_params == 1;
    ...
    fitting_bounds.BP = [0 100];  %biased prior, value can't exit the rating scale
    fitting_bounds.Opt = [0 100];  %optimism, value can't exit rating scale
    fitting_bounds.BV = [0 100];  %biased variances, can't be wider than the whole rating scale
    fitting_bounds.beta = [-Inf Inf];   %A bit arbitrary 
