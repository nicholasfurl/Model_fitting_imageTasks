function imageTasks_position_LL_trust2_v3_two_panel()
% imageTasks_position_LL_trust2_v3_two_panel
%
...
% -------------------------------------------------------------------------
try
    % Plot non-terminal positions only. In the 8-option task, the terminal
    % position is forced once it is reached and therefore is not a genuine
    % stop/continue decision in the same sense as earlier positions. The
    % tables saved above still include all positions.
    plot_positions = Position(1:end-1);
    plot_mean_nll = mean_nll_by_pos(1:end-1,:);
    plot_delta_nll = delta_mean_nll(1:end-1,:);

    % Display labels. The lines are model likelihoods, not participant groups.
    model_labels = {'Cost-to-sample model','Cut off model','Biased-prior model'};

    % Use different line styles/markers so the figure remains readable if
    % printed in greyscale, while retaining MATLAB's default colour order.
    line_specs = {'-o','--s',':^'};
    line_width = 2.0;
    marker_size = 6;

    f = figure('Color','w', ...
        'Name','Trust 2 position likelihood two-panel', ...
        'Units','pixels', ...
        'Position',[100 100 1200 430]);

    % -------------------------
    % Panel A: absolute fit
    % -------------------------
    ax1 = subplot(1,2,1);
    hold(ax1,'on'); box(ax1,'off');
    for mi = 1:num_models
        plot(ax1, plot_positions, plot_mean_nll(:,mi), line_specs{mi}, ...
            'LineWidth', line_width, 'MarkerSize', marker_size);
    end
    xlabel(ax1,'Sequence position');
    ylabel(ax1,sprintf('Mean negative log likelihood\n/ observed decision'));
    title(ax1,'A. Absolute fit');
    set(ax1, ...
        'FontName','Arial', ...
        'FontSize',12, ...
        'XTick',plot_positions, ...
        'TickDir','out');

    % -------------------------
    % Panel B: relative fit
    % -------------------------
    ax2 = subplot(1,2,2);
    hold(ax2,'on'); box(ax2,'off');
    for mi = 1:num_models
        plot(ax2, plot_positions, plot_delta_nll(:,mi), line_specs{mi}, ...
            'LineWidth', line_width, 'MarkerSize', marker_size);
    end
    plot(ax2,[min(plot_positions) max(plot_positions)],[0 0],'k:','LineWidth',1);
    xlabel(ax2,'Sequence position');
    ylabel(ax2,sprintf('\Delta mean negative log likelihood\n/ best model'));
    title(ax2,'B. Relative fit');
    set(ax2, ...
        'FontName','Arial', ...
        'FontSize',12, ...
        'XTick',plot_positions, ...
    ...
    % One legend for the full two-panel figure.
    lgd = legend(ax2, model_labels, 'Location', 'best');
    lgd.Box = 'off';

    % Make both panels use the same width and tidy the spacing.
    set(ax1,'Units','normalized');
    set(ax2,'Units','normalized');
    ax1.Position = [0.075 0.17 0.39 0.70];
    ax2.Position = [0.575 0.17 0.39 0.70];

    % Save combined figure. PNG is convenient for checking; TIFF is for the SM.
    set(f,'PaperPositionMode','auto');
    png_file = [cfg.outdir filesep 'trust2_position_likelihood_two_panel.png'];
    tif_file = [cfg.outdir filesep 'trust2_position_likelihood_two_panel.tif'];
    saveas(f, png_file);
    print(f, tif_file, '-dtiff', '-r600');
    fprintf('Wrote combined likelihood figure:\n  %s\n  %s\n', png_file, tif_file);
