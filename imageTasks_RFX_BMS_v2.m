...
%
% Nicholas / ChatGPT draft v2, June 2026.
% =========================================================================

function imageTasks_RFX_BMS_v2()
...
% Initialise from the first returned structure. Some MATLAB versions do not
% allow indexed assignment of a populated structure into struct([]) when the
% target has no fields.
for f = 1:numel(cfg.file_paths)
    fprintf('\nLoading %s\n', cfg.study_names{f});
    D = load_image_task_evidence(cfg.file_paths{f}, cfg.study_names{f}, cfg.study_short{f}, cfg);
    if f == 1
        dataset = D;
    else
        dataset(f) = D;
