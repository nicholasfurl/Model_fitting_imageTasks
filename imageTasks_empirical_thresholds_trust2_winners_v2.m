function imageTasks_empirical_thresholds_trust2_winners_v2()
% imageTasks_empirical_thresholds_trust2_winners_v2
%
...
cfg.n_bins = 20;                   % finer bins help when choices only rise near the upper tail
...
% Logistic function used for threshold extraction.
% 'upper_free' matches the old analysis spirit better when psychometric
% curves have low maximum choice rates: p = upper / (1 + exp(-(a + b*x))).
% The threshold is the inflection point, where p = upper/2. This avoids
% forcing the threshold to be the bin where p(stop)=.5 when the observed
% curve never approaches .5.
% 'upper_1' is the conventional 0-1 logistic used in v1.
cfg.logistic_model = 'upper_free';
...
% Save tables.
writetable(D, [cfg.outdir filesep 'trust2_empirical_threshold_trials_winners_v2.csv']);
writetable(Binned, [cfg.outdir filesep 'trust2_empirical_threshold_binned_winners_v2.csv']);
writetable(Thresholds, [cfg.outdir filesep 'trust2_empirical_threshold_pse_winners_v2.csv']);

fprintf('\nWrote:\n');
fprintf('  %s\n', [cfg.outdir filesep 'trust2_empirical_threshold_trials_winners_v2.csv']);
fprintf('  %s\n', [cfg.outdir filesep 'trust2_empirical_threshold_binned_winners_v2.csv']);
fprintf('  %s\n', [cfg.outdir filesep 'trust2_empirical_threshold_pse_winners_v2.csv']);
...
save([cfg.outdir filesep 'trust2_empirical_threshold_analysis_winners_v2.mat'], ...
...
for gi = 1:numel(groups)
    for si = 1:numel(sources)
        for pi = 1:numel(cfg.positions)
            ...
            [pse, slope, intercept, upper, extrap, ok, nll] = fit_increasing_logistic_threshold(x, success, trials, cfg);
            ...
            UpperAsymptote(end+1,1) = upper; %#ok<AGROW>
            ThresholdIsExtrapolated(end+1,1) = extrap; %#ok<AGROW>
...
Thresholds = table(BestFitModel, Source, Position, ThresholdPSE, Slope, ...
    Intercept, UpperAsymptote, ThresholdIsExtrapolated, NTrials, NSuccess, NFailure, FitOK, NegLogLik);

end

function [pse, slope, intercept, upper, extrapolated, ok, nll] = fit_increasing_logistic_threshold(x, success, trials, cfg)

pse = NaN; slope = NaN; intercept = NaN; upper = NaN;
extrapolated = false; ok = false; nll = NaN;
...
% Initial threshold near the midpoint of the fitted curve. For the
% upper-free model this is upper/2, not .5.
if strcmpi(cfg.logistic_model, 'upper_free')
    init_upper = min(max(max(p_obs)*1.15, 0.05), 0.98);
    target_p = init_upper/2;
    [~,closest] = min(abs(p_obs - target_p));
    init_pse = x(closest);
    init_slope = 0.75;
    init_intercept = -init_slope * init_pse;
    theta0 = [init_intercept, log(init_slope), logit(init_upper)];
elseif strcmpi(cfg.logistic_model, 'upper_1')
    ...
    error('Unknown cfg.logistic_model: %s', cfg.logistic_model);
end

opts = optimset('Display','off','MaxIter',10000,'MaxFunEvals',10000);
obj = @(th) logistic_nll(th, x, success, trials, cfg);
...
if strcmpi(cfg.logistic_model, 'upper_free')
    upper = logistic(theta(3)); % 0 < upper < 1
else
    upper = 1;
end
...
pse = -intercept ./ slope;
nll = fval;
extrapolated = pse < 1 || pse > cfg.n_bins;
ok = isfinite(pse) && isfinite(slope) && isfinite(upper) && isfinite(nll);

end

function nll = logistic_nll(theta, x, success, trials, cfg)
...
fprintf('====================================================================\n');
fprintf('Thresholds are logistic inflection points in value-bin units.\n');
fprintf('Logistic model: %s. For upper_free, the inflection is at fitted upper asymptote / 2.\n', cfg.logistic_model);
...
for gi = 1:numel(groups)
    ...
    for si = 1:numel(sources)
        ...
        for ri = 1:height(th)
            if th.FitOK(ri)
                fprintf('    pos %d: threshold = %.3f, slope = %.3f, upper = %.3f, trials = %d, extrapolated = %d\n', ...
                    th.Position(ri), th.ThresholdPSE(ri), th.Slope(ri), th.UpperAsymptote(ri), th.NTrials(ri), th.ThresholdIsExtrapolated(ri));
            else
...
for gi = 1:numel(groups)
    ...
    ylabel(ax,'Choice threshold (value-bin inflection point)');
...
if cfg.save_figures
    base = [cfg.outdir filesep 'trust2_empirical_thresholds_winners_v2'];
    saveas(f, [base '.png']);
...
for si = 1:numel(sources)
    for gi = 1:numel(groups)
        ...
        for pi = 1:numel(cfg.positions)
            ...
            % Overlay the fitted logistic curve, if available.
            tr = Thresholds(string(Thresholds.BestFitModel)==groups(gi) ...
                & string(Thresholds.Source)==sources(si) ...
                & Thresholds.Position==pos ...
                & Thresholds.FitOK==true,:);
            if ~isempty(tr)
                xd = linspace(1, cfg.n_bins, 200);
                yd = tr.UpperAsymptote ./ (1 + exp(-(tr.Intercept + tr.Slope*xd)));
                plot(ax, xd, yd, '-', 'Color', pos_cols(pi,:), 'LineWidth', 1.1);
            end
        end
...
if cfg.save_figures
    base = [cfg.outdir filesep 'trust2_empirical_psychometric_curves_winners_v2'];
    saveas(f, [base '.png']);
    print(f, [base '.tif'], '-dtiff', '-r600');
