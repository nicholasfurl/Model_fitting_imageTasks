...
%     payoff( find(payoff <= Generate_params.model(Generate_params.current_model).BRmid)) = 0;
    payoff( find(sampleSeries <= Generate_params.model(Generate_params.current_model).BRmid)) = 1;
    payoff( find(sampleSeries > Generate_params.model(Generate_params.current_model).BRmid)) = 100;
    payoff = sort(payoff,'descend');   %sort the sample values
